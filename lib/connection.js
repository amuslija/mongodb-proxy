...
var Connection = function(proxy, connection, logger) {
  ...
  MongoClient.connect(proxy.options.uri, {
    ...
    self.topology = self.db.topology.s.coreTopology;
...
Connection.prototype.messageHandler = function(data) {
  ...
  if(bufferIndexOf(data, ismaster, 0) != -1) {
    ...
    var reply = new ReplyMessage(self.topology.s.bson, requestId, responseTo, [ismasterResponse]);
    ...
    if(rIndex != -1) {
      ...
      var doc = self.topology.s.bson.deserialize(data.slice(rIndex, rIndex + readPreferenceDocLength));
    ...
    if(clientMessage.opCode() == 2005) {
      ...
      try {
        server = self.topology.getServers({
    ...
    var callbackFunction = function(_server, _clientMessage, _clientConnection) {
      ...
      if(clientMessage.opCode() == 2005) {
        ...
        // Get the connection
        connection = server[0].connections()[0];
        // Write the data to the connection
        if(connection.isConnected())
          connection.write(_clientMessage);
      }
      ...
      return function(err, data) {
        ...
        var responseMessage = new WireMessage(data);
        // Extract the cursor
    ...
    callbackFunction(server, data, this.connection)(null, data);
    // Get the callbacks
